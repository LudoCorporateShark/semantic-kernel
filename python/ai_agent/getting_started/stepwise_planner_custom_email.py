...
class EmailPlugin:
    ...
    def send_email(
        ...
        recipient: Annotated[list[str], "the list of email recipient(s)"],
    ) -> Annotated[str, "the output is a string"]:
        """Sends an email with the provided subject, body and recipient(s)."""

        # Ensure recipient is a list
        if isinstance(recipient, str):
            recipient = [recipient]
        elif not isinstance(recipient, list):
            print("Recipient must be a list of email addresses.", file=sys.stderr)

        msg = MIMEMultipart()
        msg["From"] = "auto@cleardata.co.uk"
        ...
        try:
            cleardata_email.send_msg_obj(recipient, msg)
            print(f"Email sent to {recipient}")
            return f"Email sent to {', '.join(recipient)} with subject: {subject} and body: {body}"
        except Exception as eem:
            error_message = f"Failed to send email to {', '.join(recipient)}: {eem}"
